# Home by Bami — E-Commerce / Ordering Site
...
Catalog, cart, checkout (pickup or delivery), order tracking, and an admin dashboard.
...
- Repo: https://github.com/kaotikus27/E-Commerce
- Local clone (Windows): `C:\Users\Public.DESKTOP-3A1LBHM\Downloads\Ai-Practice\E-Commerce`
- Local clone (Mac, per last handoff): `/Users/ehalaman/Documents/AI-PRACTICE/coffeeshop-ecommerce/E-Commerce`
- The repo has its own `README.md`, `AGENTS.md`, and `docs/` (`delivery-fulfillment.md`, `user-location-capture.md`) — those stay in the repo as the source of truth for how the code works.
> **Note:** the H2 database is in-memory and dev-only (`jdbc:h2:mem:bakerydb`) — see `handoff.md` Open Issues for the plan to move off it.
## Quick Start
...
Further reading
./docs/delivery-fulfillment.md — how a delivery order goes from checkout quote to a dispatched rider, including the Lalamove webhook signature verification.
./docs/user-location-capture.md — how the checkout flow resolves an exact delivery pinpoint (search → disambiguation → draggable map → structured rider notes).
...
KOS files in this folder
This KOS project folder holds cross-session continuity (what's done, what's next, durable decisions)
rather than duplicating the repo's own documentation:

PROJECT-CONTEXT.md — objective, users, constraints, current phase.
memory.md — durable architecture decisions and constraints that should survive across sessions and machines.
handoff.md — current state, open issues, next action. Replaced each session; prior states archived under docs/history/handoffs/.
DECISIONS.md — decision log.
backlog.md — open, not-yet-scheduled work.